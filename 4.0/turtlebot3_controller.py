...
class Turtlebot3Controller(Node):
    ...
    def GoHome(self, sets):
        cordination = sets
        if self.init_odom_state is True:
            if cordination == [[0, 0], "r"]:
                if self.state == 0:
                    self.TurnTo(90)
                elif self.state == 1:
                    self.GTNN(1)
